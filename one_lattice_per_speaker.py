...
def main():
    parser = argparse.ArgumentParser(description='Read in a txt file containing paths to lattices (*.npz) and extract one path for each speaker')
    parser.add_argument('-f', '--file-to-process', required=True, type=str,
                        help='The path to the txt file containing the dataset from which to extract file paths')
    parser.add_argument('-d', '--dst-name', required=True, type=str,
    ...
    for lat_path in lattice_list:
        ...
        if not speaker in speaker_dict.keys():
            ...
            speaker_dict[speaker] += 1
        if speaker_dict[speaker] <= NUM_LATTICES_PER_SPEAKER:
